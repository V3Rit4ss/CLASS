...
m1 <- matrix(x, nrow=3, ncol=5, byrow =T)   #byrow = T 행 우선 저장.
m1
...
#(1) 1, 3, 5, 7, 9
(a <- seq(from= 1, to=10, by=2)) #seq(1,9,2)

#(2) 1, 1, 1, 1, 1
(a <- rep(1:1, each=5))#rep(1,times=5)

#(3) 1, 2, 3, 1, 2, 3, 1, 2, 3
(a <- rep(1:3, times=3)) #rep(seq(1,3),3)


#(4) 1, 1, 2, 2, 3, 3, 4, 4
(a <- rep(1:4, each=2)) #rep(seq(1,4), each=2)


#[문제4] 1부터 10 까지 출력하는데 3씩 증가 되는 형태로(1 4 7 10)저장되는 벡터를 정의하여 v3 변수에 저장한다.
#                                       (또한 각각 값마다 "A", "B", "C", D" 라는 이름을 부여한다.)
(v3 <- (seq(1, 10, by=3)))  # v3 <- seq(1,10,3)
names(v3) <-c('A','B','C','D') 
v3
...
#      값이 가장 큰 요일의 명칭을 출력한다.  which.max()   order도 가능.
#      값이 가장 작은 요일의 명칭을 출력한다.   which.min()  which.max(-x) 가능.
...
#m1 <- matrix(x, nrow=3, ncol=5, byrow =T)
(n1 <- matrix(1:3)) #n1 <- c(1,2,3)
(n2 <- matrix(4:6)) #n2 <- c(4,5,6)
(n3 <- matrix(7:9)) #n3 <- c(7,8,9)
...
(m2 <- t(m1))   #m2 <- matrix(c(1:9), nrow =3, byrow = T)
...
#	제품명	가격	판매량 x=속성 , 변수이름 , 열이름 이라 불리운다.
#	사과	1800	24
#	딸기	1500	38
#	수박	3000	13
제품명 <- c('사과','딸기','수박')
...
# c list 를 쓰게되면 apply 계열 함수 이용불가. 
...
#drop=FALSE 는 데이터타입으로 추출.


#[문제 23] iris 데이터의 종별 데이터 개수 출력한다
#iris.species <- split(iris. iris)
table(iris$Species)
