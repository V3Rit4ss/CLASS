...
SELECT ENAME , SUM(SAL) FROM EMP;  -- 단일행과 그룹행은 같이 쓸수없다.
SELECT DEPTNO, SUM(SAL) FROM EMP GROUP BY DEPTNO;  -- 셀렉 DEPTNO 와 그룹바이 DEPTNO 
...
SELECT COUNT(*) FROM EMP; --로우(행)수 전체
SELECT AVG(SAL), SUM(SAL)/COUNT(SAL) FROM EMP;
SELECT COMM FROM EMP; --(300, 500 1400 , 0)/14? /4? 로 하려는데 오라클에서는 어떻게 할까?
SELECT AVG(COMM) FROM EMP; --그룹함수는 NULL값을 제외한다.
SELECT COUNT(COMM), SUM(COMM), AVG(COMM)FROM EMP; --위하 동문.
--EMP 테이블에서 SAL 의 평균,합,최솟값,최대값,분산, 표준편차
SELECT ROUND(AVG(SAL)), SUM(SAL) ,MIN(SAL),MAX(SAL),ROUND(VARIANCE(SAL)),ROUND(STDDEV(SAL)) FROM EMP; 

--부서번호 별  급여 최대치 --그룹절에는 별명 못함.
SELECT DEPTNO "부서번호" ,MAX(SAL) FROM EMP GROUP BY DEPTNO ORDER BY "부서번호" ;
--그룹함수는 숫자형,문자형,날짜형 모두 사용가능
--부서번호별로 최초 입사년도 , 최근입사년도
SELECT DEPTNO, MIN(HIREDATE), MAX(HIREDATE) FROM EMP GROUP BY DEPTNO;
SELECT MIN(ENAME), MAX(ENAME) FROM EMP;  --알파벳 별로 나옴.
SELECT COUNT(JOB) FROM EMP; --JOB 필드의 갯수. 14개 가 나온다.
SELECT COUNT(DISTINCT JOB) FROM EMP; --JOB의 갯수 DISTINCT 를 써서 중복을 없애준다.

--예제.
--­	(결과) 80년12월17일 최초입사  :14,620일째 83년01월12일 최근입사 :13,864일째
SELECT MIN(HIREDATE)||' 최초 입사 : '||TRUNC(SYSDATE-MIN(HIREDATE))||'일째',
        MAX(HIREDATE)||' 최근 입사 : '||TRUNC(SYSDATE-MAX(HIREDATE))||'일째' FROM EMP;
--­	(결과) 80년12월17일 최초입사  :14,620일째 83년01월12일 최근입사 :13,864일째
SELECT MIN(HIREDATE)||' 최초 입사 : '||TO_CHAR(TRUNC(SYSDATE-MIN(HIREDATE)), '99,999')||'일째',
            MAX(HIREDATE)||' 최근 입사 : '||TO_CHAR(TRUNC(SYSDATE-MAX(HIREDATE)), '99,999')||'일째' FROM EMP;
...
--SELECT ENAME , HIREDATE FROM EMP WHERE HIREDATE=(SELECT MAX(HIREDATE) FROM EMP); -- 서브쿼리

--부서 번호별 사원수, 최대급여 , 최소급여, 평균급여
SELECT DEPTNO , COUNT(*), MAX(SAL), MIN(SAL), ROUND(AVG(SAL)) FROM EMP GROUP BY DEPTNO;

--부서명 별, 사원수, 최대급여,최소급여, 평균급여,
SELECT DNAME , COUNT(*) , MAX(SAL), MIN(SAL) ,ROUND(AVG(SAL)) FROM EMP E, DEPT D  WHERE E.DEPTNO=D.DEPTNO GROUP BY DNAME;

SELECT DEPTNO, JOB, SUM(SAL) FROM EMP GROUP BY DEPTNO, JOB ORDER BY DEPTNO;
--SELECT [DEPTNO, JOB], SUM(SAL) FROM EMP GROUP BY [DEPTNO, JOB] ORDER BY DEPTNO;  [] 안의 내용이 같아야한다.
...
--DEPTNO 별로 평균급여 (AVG(SAL)가 2000이상인 부서만 출력)
SELECT DEPTNO, AVG(SAL)  FROM EMP WHERE AVG(SAL)>2000 GROUP BY DEPTNO; --오류.
...
SELECT DEPTNO , SUM(DECODE(JOB, 'CLERK', SAL, 0)) "CLERK", 
    ...
    SUM(DECODE(JOB, 'SALESMAN',SAL, 0))"SALESMAN" FROM EMP GROUP BY DEPTNO;  --피벗테이블
...
SELECT DEPTNO , SUM(DECODE(JOB, 'CLERK', SAL, 0)) "CLERK", 
    ...
    SUM(DECODE(JOB, 'PRESIDENT',SAL, 0))"PRESIDENT",
    SUM(DECODE(JOB, 'ANALYST',SAL, 0))"ANALYST",
    SUM(DECODE(JOB, 'SALESMAN',SAL, 0))"SALESMAN" , SUM(SAL) "부서별소개" FROM EMP GROUP BY ROLLUP(DEPTNO);
...
SELECT JOB , SUM(SAL) "SUMSAL" FROM EMP GROUP BY JOB HAVING SUM(SAL) > 5000 ORDER BY "SUMSAL" DESC; --오더바이절에는 별명 가능.
...
SELECT DEPTNO , SUM(DECODE(JOB, 'CLERK', SAL, 0)) "CLERK",     --DECODE(JOB, 'CLERK', SAL, 0)  가로안의 값이 0으로 나와라 OR 1로 나와라.
    SUM(DECODE(JOB, 'MANAGER',SAL, 0)) "MANAGER", 
    SUM(DECODE(JOB, 'PRESIDENT',SAL, 0))"PRESIDENT",
    SUM(DECODE(JOB, 'ANALYST',SAL, 0))"ANALYST",
    SUM(DECODE(JOB, 'SALESMAN',SAL, 0))"SALESMAN" , SUM(SAL) "부서별소개" FROM EMP GROUP BY ROLLUP(DEPTNO);
...
--SCOTT		2
--……
SELECT E1.ENAME, COUNT(E2.ENAME)+1 "등수" FROM EMP E1, EMP E2 WHERE E1.SAL<E2.SAL(+) GROUP BY E1.ENAME ORDER BY "등수";
